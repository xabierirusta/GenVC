...
workflow vc_somatic_wes {
    ...
    gnomad_ann          // gnomAD database annotations
    pon_hg38            // Panel of Normals of hg38 for mutect2
    cleaned_ref_genome  // Cleaned reference genome fasta
    dbsnp_ann           // dbSNP database annotations
    refFlat             // refFlat.txt file
    bed_files           // Targets BED related files
    targets_bed         // BED file containing regions of interest
    access_bed          // Access BED required by CNVKit
    cache_dir           // Annotation cache directory for Ensembl VEP
    normal              // Markduplicates BAM and BAI files for the normal control
    vcf_files           // VCF files required in the worflow
    ...
    vc_cnvkit_wes(mdup, ref_files, bed_files, refFlat, access_bed, targets_bed, cleaned_ref_genome, normal_channel)
    visualize_cnvkit(vc_cnvkit_wes.out.cnvkit_output)
    // Intermediate Channels needed in the heatmap generation
    cns_files = vc_cnvkit_wes.out.cnvkit_output
    ...
    vcf_mix = filter_mutect2.out.filt_mutect2_vcf
        | mix(vc_cnvkit_wes.out.cnvkit_vcf, filter_delly.out.filt_delly_vcf)
        | groupTuple(size: 3)
        | map { tuple ->
